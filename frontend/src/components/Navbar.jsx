...
function Navbar({ customClasses }) {
    ...
    return (
        <>
            <nav className={classnames(customClasses || "fixed text-black dark:text-white top-0 left-0 right-0 dark:bg-gray-800 bg-white shadow-md aleo-regular z-50 select-none")}>
                {/* Navbar for sm screen */}
                <div className="container mx-0 px-6 py-3 flex justify-between items-center dark:bg-gray-800">
                    {/* <Link to="/" className="text-4xl font-bold hover:text-indigo-600">HIRIZONE</Link> */}
                    <img src={logo} alt="logo" className="w-[150px] h-[ 100px] " />
                    <img src={darklogo} alt="logo" className="w-[150px] h-[ 100px] " />
                    ...
                    <div ref={MenuRef} className={`lg:hidden fixed top-0 right-0 bg-white shadow-md p-4 w-64 z-10 transition-all duration-300 ${showMenu ? 'translate-x-0' : 'translate-x-full'}`}>
                        ...
                        <div className="flex flex-col">
                            ...
                            <Link to="/opensource" className="text-gray-700 dark:text-white hover:text-indigo-600 py-2 flex items-center">
